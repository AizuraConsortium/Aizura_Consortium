import { createQueryBuilder, type WriteQueryBuilder } from '../../shared/services/supabase/queryBuilderFactory.js';
...
function createMessagesRepository(client?: SupabaseClient) {
  ...
  async function insertMessage(
    ...
    const message = await builder.create<Message>('messages', {
      topic_id: topicId,
      agent_id: agentId,
  ...
  async function getRecentMessages(
    ...
    return builder.getMany<Message>(
      'messages',
  ...
  async function getMessagesInCurrentTick(
    topicId: string,
    agentIds: AgentId[]
  ): Promise<Message[]> {
    return builder.getMany<Message>(
  ...
  async function markMessageAsSelected(
    messageId: string
  ): Promise<void> {
    await builder.updateById('messages', messageId, { selected: true });
  }

  async function getMessageWithRelations(messageId: string): Promise<Message & { tool_calls?: MessageToolCall[]; proposed_actions?: any[] }> {
    const message = await builder.getMany<Message>('messages', { id: messageId }, { limit: 1 });
    if (!message || message.length === 0) {
      throw new Error(`Message not found: ${messageId}`);
    }

    const toolCalls = await builder.getMany<MessageToolCall>('message_tool_calls', { message_id: messageId });
  ...
  function reconstructMessageBody(
    message: Message,
  ...
  return {
    insertMessage,
    getRecentMessages,
    getMessagesInCurrentTick,
    markMessageAsSelected,
    getMessageWithRelations,
    reconstructMessageBody,
    getTopicMessages,
    getMessageById,
  };
}

const defaultRepository = createMessagesRepository();
...
export { createMessagesRepository };
